...
const bulkRecommendButton = document.querySelector("#appeal-bulk-recommend");
...
const heroStatus = document.querySelector("#appeal-hero-status");
const syncPill = document.querySelector("#appeal-sync-pill");
const newCount = document.querySelector("#appeal-new-count");
const forwardedCount = document.querySelector("#appeal-forwarded-count");

const STORAGE_KEY = "nursetrack-ci-student-appeals";
const archivedStatuses = new Set(["recommended", "chair-approved", "rejected"]);
...
function normalizeAppealStatus(status) {
  ...
  return statusMeta[status] ? status : "new";
}
...
function getMergedAppeals() {
  ...
  return appeals
    .map((appeal) => {
      const mergedAppeal = { ...appeal, ...(overrides[appeal.id] || {}) };
      return {
        ...mergedAppeal,
        status: normalizeAppealStatus(mergedAppeal.status)
      };
    });
}

function getAppeals() {
  return getMergedAppeals().filter((appeal) => !archivedStatuses.has(appeal.status));
}
...
function isAppealLocked(appeal) {
  return appeal.status === "recommended" || appeal.status === "chair-approved";
}
...
function updateSummary(records) {
  const openRecords = records.filter((appeal) => appeal.status === "new");
  const newRecords = records.filter((appeal) => appeal.status === "new");
  const forwardedRecords = records.filter((appeal) => appeal.status === "recommended");

  updateText(newCount, newRecords.length);
  updateText(forwardedCount, forwardedRecords.length);
  updateText(heroStatus, `${openRecords.length} open appeal${openRecords.length === 1 ? "" : "s"}`);
  updateText(syncPill, `${openRecords.length} open`);
}
...
function syncSelectedAppeals(records) {
  const validIds = new Set(records.filter((appeal) => !isAppealLocked(appeal)).map((appeal) => appeal.id));
  selectedAppealIds = new Set(Array.from(selectedAppealIds).filter((id) => validIds.has(id)));
}

function updateBulkControls() {
  const actionableVisibleAppeals = currentVisibleAppeals.filter((appeal) => !isAppealLocked(appeal));
  const selectedCount = selectedAppealIds.size;
  const allActionableSelected = actionableVisibleAppeals.length > 0 &&
    actionableVisibleAppeals.every((appeal) => selectedAppealIds.has(appeal.id));

  updateText(selectedCountLabel, `${selectedCount} selected`);

  if (selectAllButton) {
    selectAllButton.textContent = allActionableSelected ? "Deselect all appeals" : "Select all appeals";
    selectAllButton.disabled = actionableVisibleAppeals.length === 0;
  }

  if (bulkRecommendButton) {
    bulkRecommendButton.disabled = selectedCount === 0;
  }
}
    ...
    <article class="appeal-card ${isSelected ? "is-selected" : ""}" data-appeal-id="${appeal.id}">
      <div class="appeal-card-head">
        <div class="appeal-student-cell">
          <label class="appeal-select-control" aria-label="Select appeal from ${appeal.student}">
            <input type="checkbox" data-appeal-select="${appeal.id}" aria-label="Select appeal from ${appeal.student}"${isSelected ? " checked" : ""}${isLocked ? " disabled" : ""}>
          </label>
      ...
      <div class="appeal-card-body">
        ...
        <div class="appeal-detail-grid">
          ...
          <div class="appeal-detail-note">
            <p class="section-kicker">Supporting note</p>
            <p>${appeal.evidence}</p>
          </div>
        </div>
...
function renderAppeals() {
  ...
  updateText(appealMessage, `Showing ${visibleRecords.length} student appeal${visibleRecords.length === 1 ? "" : "s"}.`);
...
selectAllButton?.addEventListener("click", () => {
  ...
  if (allActionableSelected) {
    actionableVisibleAppeals.forEach((appeal) => selectedAppealIds.delete(appeal.id));
  } else {
    actionableVisibleAppeals.forEach((appeal) => selectedAppealIds.add(appeal.id));
  }

  renderAppeals();
});

bulkRecommendButton?.addEventListener("click", () => {
  ...
  selectedAppealIds.forEach((id) => {
    updateAppeal(id, { status: "recommended" });
  });
  selectedAppealIds.clear();
  renderAppeals();
  appealMessage.textContent = `${selectedCount} appeal${selectedCount === 1 ? "" : "s"} accepted and forwarded to Chair.`;
...
appealList?.addEventListener("click", (event) => {
  ...
  const recommendButton = event.target.closest("[data-appeal-recommend]");
  ...
  if (recommendButton) {
    updateAppeal(recommendButton.dataset.appealRecommend, { status: "recommended" });
    selectedAppealIds.delete(recommendButton.dataset.appealRecommend);
    renderAppeals();
    appealMessage.textContent = "Appeal accepted and forwarded to Chair.";
    appealMessage.classList.add("is-success");
    return;
  }
  ...
  if (rejectButton) {
    ...
    renderAppeals();
    appealMessage.textContent = "Appeal rejected and removed from the queue.";
